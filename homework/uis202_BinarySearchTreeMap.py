class BinartSearchTreeMap:
        ...
        if(self.is_empty() == True):
            ...
            while(cursor is not None):
                ...
                if(key < cursor.item.key):
                    cursor.left_elements += 1
                    cursor = cursor.left
                else:
                    cursor.right_elements += 1
                    cursor = cursor.right
        ...
        if(node_to_delete is self.root):
            ...
            if(num_children == 0):
                ...
                if(node_to_delete is parent.left):
                    parent.left_elements = 0
                    parent.left = None
                else:
                    parent.right_elements = 0
                    parent.right = None
                ...
                if(node_to_delete is parent.left):
                    parent.left_elements -= 1
                    parent.left = child
                else:
                    parent.right_elements -= 1
                    parent.right = child
                child.parent = parent
                ...
                max_in_left.left_elements = (node_to_delete.left_elements - 1)
                max_in_left.right_elements = node_to_delete.right_elements
    ...
    def get_ith_smallest(self, i):
        ...
        if(not 1 <=  i <= self.n):
            raise IndexError(str(i) + "is not in range")
        prev = None
        elem = self.root
        elem_pos = (self.root.left_elements + 1) 
        while(elem is not None):
            if(elem_pos == i):
                ...
                if(elem is None):
                    return prev.item.key
                elem_pos -= (elem.right_elements + 1)
